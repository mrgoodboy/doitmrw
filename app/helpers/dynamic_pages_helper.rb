module DynamicPagesHelper
  ...
  def adjust_like()
    ...
    timeDecay1 = uct*(get_current_time - content1.created_at)
    timeDecay2 = uct*(get_current_time - content2.created_at)
    edge1.weight = edge1.weight + ucc - timeDecay1
    if edge1.weight < 0
      edge1.weight = 0
    end
    edge1.save

    edge2.weight = edge2.weight + ucc - timeDecay1
    if edge2.weight < 0
      edge2.weight = 0
  end
  edge2.save

  edge3.weight = edge3.weight + avgc*get_average(content2) - timeDecay2 + 
  end

  def next_content(category) #returns an id number of the next page
    #Content.order("RAND()").first
    if rand > 0.2
      next_page = new_content(category)
    else 
      next_page = old_content(category)
    end
    return next_page
    
  end

  def new_content(category)
    if count_new_content()==0
      return pick_random_content()
    else
    #graham will do this shit
    #randomly picks a piece of new content flagged as new
    #stored in next_page which is the next content
  end
  
    if check_edge(current_user.Id,next_page.Id)
      return pick_random_content(category)
    else
      return next_page
    end
  ...
  def old_content(category)
    edge1 = current_user.random_edge(category)
    content1 = edge1.Content
    edge2 = content1.random_edge(category)
    user2 = edge2.User
    edge3 = user2.random_edge(category)
    content2 = edge3.Content
    if check_edge(content2.Id,current_user.Id)
      return new_content(category)
    else
      session[:edges] => [edge1,edge2,edge3]
      return content2
    end
  ...
  def pick_random_content(category)
    #pick a random content from all, can be completely random
  end
